...
if tf.__version__[0]=='1':
    tf.enable_eager_execution() # for tf1
...
def detect_face(img, Pnet16 ,Rnet24 ,Onet36  ):
    ...
    t3 = 0
    minl=np.amin([h, w])
    m=16.0/20 # 20是MTCNN使用 沿用
    minl=minl*m
    # create scale pyramid
    scales=[]
    temp = 0
    while minl>= 20: # 20是MTCNN使用 沿用

        scales += [m*np.power(factor, factor_count)]    #圖像金字塔 不斷乘factor 保存數據到scales裡面
        minl = minl*factor
        factor_count += 1
    # first stage
    for scale in scales: #把每一個圖像金字塔的圖片丟給Pnet
...
#Rnet開始
    ...
    if numbox>0:
        ...
        if total_boxes.shape[0]>0:
            pick = nms(total_boxes, 0.7, 'Union') # NMS IoU
            total_boxes = total_boxes[pick,:]
            total_boxes = bbreg(total_boxes.copy(), np.transpose(mv[:,pick])) #做bbox regression
            total_boxes = rerec(total_boxes.copy()) #把框框變成正方形
    ...
    if numbox>0:
        ...
        if total_boxes.shape[0]>0:
            ...
            pick = nms(total_boxes.copy(), 0.7, 'Min') #NMS IoM
            total_boxes = total_boxes[pick,:]
    numbox = total_boxes.shape[0]
...
def generateBoundingBox(imap, reg, scale, t):
    ...
    stride=2  # Pnet有幾個maxpooling就調幾個+1
    cellsize=16 # Pnet輸入大小為多少就調多少
...
def draw_cv2_bboxes(img, bboxes,fps,frame_count):
  for bbox in bboxes:
    score = bbox[4]
    ...
    fps_str = "FPS: %.2f" % (fps)
    frame_count += [fps]
    print(np.mean(np.array(frame_count)))
    cv2.putText(img, fps_str, (25, 25),
...
if __name__ == "__main__":
    ...
    while True:
        ...
        draw_cv2_bboxes(img, bboxes, fps, frame_count)
