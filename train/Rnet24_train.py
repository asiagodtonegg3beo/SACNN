...
def bbox_mse(y_true, y_pred):      #fixed_ver1   
    ...
    #tf.print('target_picked',target_picked)
...
def extract_features(example, image_size):
    ...
    image_24 = tf.image.resize(image, [24,24])  
    image_24 = tf.image.random_contrast(image_24, lower=0.5, upper=1.5)
    image_24 = tf.image.random_brightness(image_24, max_delta=0.2)
    image_24 = tf.image.random_hue(image_24,max_delta= 0.2)
    image_24 = tf.image.random_saturation(image_24,lower = 0.5, upper= 1.5)
    #tf.print('roi1',roi1)
    if random_num == 1:
        image_24 = tf.image.flip_left_right(image_24)  # 隨機翻左翻右
    ...
    image_24 = (tf.cast(image_24, tf.float32)-127.5) / 128
    ...
    return image_24, (label_cls1, roi_com1)
...
if __name__ == "__main__":
    ...
    tfrecords_path = 'tfdata_24_1040694_1bbox.tfrecords' ## 改
    ...
    path_name = 'net24_'+str(epochs_num)+'_epochs_'+str(dataset_num) ## 檔案名稱更改
  
    model = model.Rnet24_DepthConv()
    # 預訓練檔
    #pretrain_ckpt_path = ("ckpt\\"+"Pnet16_300_epochs_1910768net16_2p_Depthwise"+"\\cp-{epoch:04d}.ckpt".format(epoch = 95))
 ...
 #-------------------------------------------------------------------------------------------------------------------   
    ...
    '''  
#---------------------------------------------------------------------------------------------------------------------
    file_name = (path_name[0:(len(path_name))]+"net24_6m5d_de")  ## 檔案名稱更改
